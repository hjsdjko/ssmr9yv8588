<template>
	<div class="main-content" :style='{"padding":"0px","margin":"0 auto","color":"#333","borderRadius":"0 0 10px 10px","background":"rgba(255,255,255,.0)","width":"96%","fontSize":"16px"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0px","margin":"20px 0 0","overflow":"hidden","color":"#666","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #dcd6c3","padding":"10px 20px","margin":"0 30px 30px 0","alignItems":"center","borderRadius":"8px","flexWrap":"wrap","background":"rgba(251,249,243,.9)","display":"flex","width":"auto","fontSize":"18px","justifyContent":"center"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">宿舍名称</label>
						<el-input v-model="searchForm.sushemingcheng" placeholder="宿舍名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">学生姓名</label>
				...
				<el-row class="actions" :style='{"border":"1px solid #dcd6c3","padding":"10px 20px","margin":"0 0 30px","color":"#666","alignItems":"center","display":"flex","-webkitPerspective":"320px","justifyContent":"center","borderRadius":"8px","flexWrap":"wrap","flex":"1","background":"rgba(251,249,243,.9)","width":"auto","perspective":"320px","fontSize":"18px"}'>
					<el-button class="add" v-if="isAuth('sushefenpei','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('sushefenpei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"0 30px","borderColor":"#eee","margin":"30px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.69)","borderWidth":"0px 0 0 0px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('sushefenpei','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="btn8" v-if="isAuth('sushefenpei','水表')" @click="shuibiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
							...
							<el-button class="btn8" v-if="isAuth('sushefenpei','电表')" @click="dianbiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block","height":"40px"}'></span>
								电表
							</el-button>
							<el-button class="edit" v-if=" isAuth('sushefenpei','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('sushefenpei','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<shuibiaoxinxi-cross-add-or-update v-if="shuibiaoxinxiCrossAddOrUpdateFlag" :parent="this" ref="shuibiaoxinxiCrossaddOrUpdate"></shuibiaoxinxi-cross-add-or-update>
		<dianbiaoxinxi-cross-add-or-update v-if="dianbiaoxinxiCrossAddOrUpdateFlag" :parent="this" ref="dianbiaoxinxiCrossaddOrUpdate"></dianbiaoxinxi-cross-add-or-update>
...
import shuibiaoxinxiCrossAddOrUpdate from "../shuibiaoxinxi/add-or-update";
import dianbiaoxinxiCrossAddOrUpdate from "../dianbiaoxinxi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				shuibiaoxinxiCrossAddOrUpdateFlag: false,
				dianbiaoxinxiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			shuibiaoxinxiCrossAddOrUpdate,
			dianbiaoxinxiCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			shuibiaoxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shuibiaoxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shuibiaoxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shuibiaoxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			dianbiaoxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.dianbiaoxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','sushefenpei');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.dianbiaoxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.dianbiaoxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
