<template>
	<div class="main-content" :style='{"padding":"0px","margin":"0 auto","color":"#333","borderRadius":"0 0 10px 10px","background":"rgba(255,255,255,.0)","width":"96%","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px","margin":"20px 0 0","overflow":"hidden","color":"#666","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"1px solid #dcd6c3","padding":"10px 20px","margin":"0 0 30px","color":"#666","alignItems":"center","display":"flex","-webkitPerspective":"320px","justifyContent":"center","borderRadius":"8px","flexWrap":"wrap","flex":"1","background":"rgba(251,249,243,.9)","width":"auto","perspective":"320px","fontSize":"18px"}'>
					<el-button class="add" v-if="isAuth('dianbiaoxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('dianbiaoxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('dianbiaoxinxi','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						批量支付
					</el-button>
					<el-button class="btn18" v-if="isAuth('dianbiaoxinxi','打印')" type="success" @click="printJson()">
						<span class="icon iconfont icon-dayin6" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						打印
					</el-button>

					<el-button class="btn18" v-if="isAuth('dianbiaoxinxi','电费统计')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('dianbiaoxinxi','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('dianbiaoxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="edit" v-if=" isAuth('dianbiaoxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('dianbiaoxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="dianfeiChart1" style="width:100%;height:600px;"></div>
...
import AddOrUpdate from "./add-or-update";
	export default {
		...
		methods: {
			...
			payHandler(row){
				this.$storage.set('paytable','dianbiaoxinxi');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var dianfeiChart1 = echarts.init(document.getElementById("dianfeiChart1"),'macarons');
        this.$http({
            url: `dianbiaoxinxi/value/xueshengxingming/dianfei`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                dianfeiChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    dianfeiChart1.resize();
                };
            }
        });
      })
	...
	dianbiaoxinxistatusChange(e,row){
		if(row.status==0){
    ...
    async printJson() {
      ...
      for (let i=0; i < this.list.length; i++) {
          ...
          yongdianliang: this.list[i].yongdianliang,
          danjia: this.list[i].danjia,
          dianfei: this.list[i].dianfei,
